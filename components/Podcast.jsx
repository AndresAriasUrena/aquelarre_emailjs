...
const LatestPodcast = dynamic(() => import ('../components/LatestPodcast') )
import {getLatestPodcast} from '../services';
...
const Podcast = () => {
    ...
    useEffect(()=> {
        getLatestPodcast().then((result)=> {
            setLastPodcast(result);
            setDataLoaded(true);
        });
    }, []);
  ...
  return (
    <div
        className="relative  h-auto w-full -mt-8"
    >

        {dataLoaded && lastPodcast.map((podcast,index) =>(
            <LatestPodcast key={index} podcast={podcast}/>
        ))}
    </div>
  )
}
